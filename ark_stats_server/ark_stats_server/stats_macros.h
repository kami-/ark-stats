...
#include "\userconfig\ark_stats_server\log\global.h"

#define ARK_STATS_SERVER_VERSION        "0.1.0"

#define CONCAT_ADDON_PATH(FILE)         x\ark\addons\ark_stats_server\FILE
#define ADDON_PATH(FILE)                #CONCAT_ADDON_PATH(FILE)

#define SQL_BASE_PATH                   "\userconfig\ark_stats_server\sql"
#define CONCAT_SQL_PATH(FILE)           \userconfig\ark_stats_server\sql\FILE
#define SQL_PATH(FILE)                  #CONCAT_SQL_PATH(FILE)

//Need to add extra single quote becasue Arma2NET sucks at parsing https://github.com/firefly2442/Arma2NETConnectPlugin/blob/master/Arma2NETConnectPlugin/Format.cs#L112
#define SQ_TRY_EMPTY(VAL)               \
    call { \
        private ["_value", "_stringValue"]; \
        _value = VAL; \
        if (toUpper typeName _value != "STRING") then { \
            _stringValue = str _value; \
        } else { \
            _stringValue = _value; \
        }; \
        if (_stringValue == "") exitWith {"'"}; \
        _value; \
    }
